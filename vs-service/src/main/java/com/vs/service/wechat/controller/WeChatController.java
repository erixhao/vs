...
public class WeChatController {
    ...
    public String refreshMarket(@PathVariable String code){
        marketDataDownloader.downloadMarketData(Lists.newArrayList(code), TimePeriod.DAILY, -1, false);
        return "successful";
    }

    @RequestMapping("/refreshStocks/{codes}")
    public String refreshMarkets(@PathVariable String codes){
        marketDataDownloader.downloadMarketData(Lists.newArrayList(codes.split(",")), TimePeriod.DAILY, -1, false);
        return "successful";
    }
    ...
    public String checkMarketData(@PathVariable String code){
        List<HistoricalData> dataList = this.marketDataService.getMarketHistoricalData(code, TimePeriod.DAILY);
        return dataList == null ? "NULL" : dataList.size() + ":" + DateUtils.toMarketDate(dataList.get(dataList.size()-1).getDate());
    }
    ...
    public String checkMarketCacheCount(){
        int size =  this.marketDataService.size(TimePeriod.DAILY);
        long total = this.marketDataService.totalSize();
