...
public class TradingBook extends AbstractBook implements Comparable<TradingBook> {
    ...
    public void bookTrade(Order action, Position p ) {
        ...
        if ( positions > 0 && validate(tran) ) {
            //log.info("---- Make Transaction :" + action.getStock().getCode() + " { Date: " + HistoricalData.toMarketDate(action.getTradeDate()) + " Strategy: " + action.getStrategy() + " Direction: " + action.getTradeDirection().toString() + " Positions: " + positions + " Cost:" + f.format(action.getTradePrice() * positions)  + " Trade Price:" + action.getTradePrice() + " Total Capital: " + f.format(this.getCapital()) + "}");
